...
char* usemachine = "award495"; //default
...
extern volatile uint64_t extops[256];

int main(int argc, char *argv[]) {

	sprintf(title, "%s v%s pre alpha", STR_TITLE, STR_VERSION);

	printf("%s (c)2025 Mike Chambers\r\n", title);
	printf("[A portable, open source x86 PC emulator]\r\n\r\n");
	...
	while (running) {
		...
		if (goCPU) {
			if (!cpu_interruptCheck(&machine.CPU, &machine.i8259, 0)) {
				cpu_interruptCheck(&machine.CPU, &machine.i8259_slave, 1);
			}
