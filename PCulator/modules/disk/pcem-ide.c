#define IDE_TIME (10 * (timing_getFreq() / 1000000))
...
void ide_irq_raise(IDE* ide) {
    ...
    if (!(ide->fdisk & 2)) {
        ...
        i8259_doirq(mypic, (ide->board) ? 7 : 6);
        //printf("IRQ raise\n");
...
void ide_irq_update(IDE* ide) {
    if (ide->irqstat) //&& !((pic2.pend | pic2.ins) & 0x40) && !(ide->fdisk & 2))
        ...
        //picintc((ide->board) ? (1 << 15) : (1 << 14));
        i8259_clearirq(mypic, (ide->board) ? 7 : 6);
}
/**
...
static void ide_identify(IDE* ide) {
    ...
    char* hdd_model = "PCulator virt disk";
    int h;
    ...
    printf("IDENTIFY %d: tracks = %d, hpc = %d, spt = %d\n", cur_ide[ide->board], hdc[cur_ide[ide->board]].tracks, hdc[cur_ide[ide->board]].hpc, hdc[cur_ide[ide->board]].spt);
...
static off64_t ide_get_sector(IDE* ide) {
    if (ide->lba) {
        ...
        //printf("%d, %d, %d\n", ide->cylinder, ide->head, ide->sector);
...
void loadhd(IDE* ide, int d, const char* fn) {
...
void writeidew(int ide_board, uint16_t val) {
    ...
    if (ide->command == WIN_PACKETCMD) {
        ...
        if (ide->pos >= 512) {
            ...
            {
                timing_updateInterval(ide_timer[ide_board], 6 * IDE_TIME);
                timing_timerEnable(ide_timer[ide_board]);
            }
        }
    }
}
...
void writeide(int ide_board, uint16_t addr, uint8_t val) {
    ...
    case 0x1F6: /* Drive/Head */
        ...
        if (cur_ide[ide_board] != ((val >> 4) & 1) + (ide_board << 1)) {
            ...
            if (ide->reset || ide_other->reset) {
                ...
                timing_timerDisable(ide_timer[ide_board]);
                return;
            }
    ...
    case 0x1F7: /* Command register */
        ...
        case WIN_SRST: /* ATAPI Device Reset */
            ...
            timing_updateInterval(ide_timer[ide_board], 100 * IDE_TIME);
            timing_timerEnable(ide_timer[ide_board]);
            return;
        ...
        case WIN_READ_DMA:
            ...
            timing_updateInterval(ide_timer[ide_board], 200 * IDE_TIME);
            timing_timerEnable(ide_timer[ide_board]);
        ...
        case WIN_SPECIFY: /* Initialize Drive Parameters */
            ...
            {
                timing_updateInterval(ide_timer[ide_board], 30 * IDE_TIME);
                timing_timerEnable(ide_timer[ide_board]);
            }
            //                        pclog("SPECIFY\n");
            //                        output=1;
    ...
    case 0x3F6: /* Device control */
        if ((ide->fdisk & 4) && !(val & 4) && (ide->type != IDE_NONE || ide_other->type != IDE_NONE)) {
            ...
            {
                timing_updateInterval(ide_timer[ide_board], 500 * IDE_TIME);
                timing_timerEnable(ide_timer[ide_board]);
            }
...
void ide_callback_pri(void* dummy) { timing_timerDisable(ide_timer[0]); callbackide(0); }

void ide_callback_sec(void* dummy) { timing_timerDisable(ide_timer[1]); callbackide(1); }

void ide_write_pri(void* priv, uint16_t addr, uint8_t val) { writeide(0, addr, val); }
void ide_write_pri_w(void* priv, uint16_t addr, uint16_t val) { writeidew(0, val); }
void ide_write_pri_l(void* priv, uint16_t addr, uint32_t val) { writeidel(0, val); }
uint8_t ide_read_pri(void* priv, uint16_t addr) { return readide(0, addr); }
uint16_t ide_read_pri_w(void* priv, uint16_t addr) { return readidew(0); }
uint32_t ide_read_pri_l(void* priv, uint16_t addr) { return readidel(0); }

void ide_write_sec(void* priv, uint16_t addr, uint8_t val) { writeide(1, addr, val); }
void ide_write_sec_w(void* priv, uint16_t addr, uint16_t val) { writeidew(1, val); }
void ide_write_sec_l(void* priv, uint16_t addr, uint32_t val) { writeidel(1, val); }
uint8_t ide_read_sec(void* priv, uint16_t addr) { return readide(1, addr); }
uint16_t ide_read_sec_w(void* priv, uint16_t addr) { return readidew(1); }
uint32_t ide_read_sec_l(void* priv, uint16_t addr) { return readidel(1); }
...
void ide_pri_enable() {
    ...
    ports_cbRegister(0x1f0, 8, ide_read_pri, ide_read_pri_w, ide_write_pri, ide_write_pri_w, NULL);
    ports_cbRegister(0x3f6, 1, ide_read_pri, NULL, ide_write_pri, NULL, NULL);
}
...
void ide_sec_enable() {
    ...
    ports_cbRegister(0x170, 8, ide_read_sec, ide_read_sec_w, ide_write_sec, ide_write_sec_w, NULL);
    ports_cbRegister(0x376, 1, ide_read_sec, NULL, ide_write_sec, NULL, NULL);
}
...
void* ide_init(I8259_t* pic) {
    ...
    ide_timer[0] = timing_addTimer((void*)ide_callback_pri, NULL, 1, TIMING_DISABLED);
    ide_timer[1] = timing_addTimer((void*)ide_callback_sec, NULL, 1, TIMING_DISABLED);
...
void ide_reset_devices() {
    if (ide_drives[0].type != IDE_NONE || ide_drives[1].type != IDE_NONE) {
        ...
        {
            timing_updateInterval(ide_timer[0], 500 * IDE_TIME);
            timing_timerEnable(ide_timer[0]);
        }
    ...
    if (ide_drives[2].type != IDE_NONE || ide_drives[3].type != IDE_NONE) {
        ...
        {
            timing_updateInterval(ide_timer[1], 500 * IDE_TIME);
            timing_timerEnable(ide_timer[1]);
        }
