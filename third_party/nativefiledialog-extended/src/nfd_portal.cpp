...
#define NFD_PORTAL_HAS_GETRANDOM_HEADER 1
#else
#define NFD_PORTAL_HAS_GETRANDOM_HEADER 0
#endif
#else
#include <sys/random.h>
#define NFD_PORTAL_HAS_GETRANDOM_HEADER 1
...
#if !NFD_PORTAL_HAS_GETRANDOM_HEADER
#include <sys/syscall.h>
#if defined(__NR_getrandom)
#define NFD_PORTAL_GETRANDOM_SYSCALL __NR_getrandom
#elif defined(SYS_getrandom)
#define NFD_PORTAL_GETRANDOM_SYSCALL SYS_getrandom
#endif
#endif
#else
#define NFD_PORTAL_HAS_GETRANDOM_HEADER 0
...
nfdpathsetsize_t CountMessageIterElements(DBusMessageIter& array_iter) {
...
ssize_t ReadRandomFromUrandom(unsigned char* buf, size_t amount) {
    ...
    while (total < amount) {
        ssize_t res = read(fd, buf + total, amount - total);
        if (res == -1) {
            if (errno == EINTR) {
                continue;
            }
...
ssize_t GetRandomBytes(unsigned char* buf, size_t amount) {
#if defined(__linux__) && NFD_PORTAL_HAS_GETRANDOM_HEADER
    ssize_t res = getrandom(buf, amount, 0);
    if (res != -1 || errno != ENOSYS) {
        return res;
    }
#elif defined(__linux__) && defined(NFD_PORTAL_GETRANDOM_SYSCALL)
    ssize_t res = syscall(NFD_PORTAL_GETRANDOM_SYSCALL, buf, amount, 0);
    if (res != -1 || errno != ENOSYS) {
        return res;
    }
#endif

    return ReadRandomFromUrandom(buf, amount);
}
...
nfdpathsetsize_t ReadResponseUrisUncheckedGetArraySize(DBusMessage* msg) {
    ...
    ReadDict(iter, "uris", [&sz](DBusMessageIter& uris_iter) {
        sz = CountMessageIterElements(uris_iter);
        return NFD_OKAY;
    });
    return sz;
}
...
char* Generate64RandomChars(char* out) {
    ...
    while (amount > 0) {
        ...
        ssize_t res = GetRandomBytes(buf, amount);
        if (res == -1) {
            if (errno == EINTR)
                continue;
            else
