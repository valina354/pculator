...
int vga_init() {
	...
	debug_log(DEBUG_DETAIL, "[VGA] Initializing VGA video device\r\n");
	...
	frontend_video_present((uint32_t*)vga_framebuffer, 640, 400, 1024 * (int)sizeof(uint32_t));
	...
	if (host_create_thread(vga_renderThread, NULL) != 0) {
	...
	memory_mapCallbackRegister(0xA0000, 0x20000,
		(void*)vga_readmemory, NULL, NULL,
		(void*)vga_writememory, NULL, NULL,
		NULL);

	if (utility_loadFile(VBIOS, 32768, "roms/video/et4000.bin")) {
		return -1;
	}
	memory_mapRegister(0xC0000, 0x4000, VBIOS, NULL);
	memory_mapRegister(0xC4000, 0x4000, &VBIOS[0x4000], NULL);
...
void vga_update(uint32_t start_x, uint32_t start_y, uint32_t end_x, uint32_t end_y) {
	...
	if (vga_attrd[0x10] & 1) { //graphics mode enable
		...
		blinkenable = 0;
		fontbase = 0; //vga_fontbases[vga_seqd[0x03]]; TODO: fix this
	...
	case VGA_MODE_TEXT:
		...
		cursorloc = (cursorloc - startaddr) & 0xFFFF;
		...
		for (scy = start_y; scy <= end_y; scy++) {
			...
			for (scx = start_x; scx <= end_x; scx++) {
				...
				if ((y == cursor_y) && (x == cursor_x) &&
					((uint32_t)(scy % maxscan) >= (vga_crtcd[VGA_REG_DATA_CURSOR_BEGIN] & 31)) &&
					((uint32_t)(scy % maxscan) <= (vga_crtcd[VGA_REG_DATA_CURSOR_END] & 31)) &&
...
void vga_renderThread(void* dummy) {
	...
	while (running) {
		...
		if (vga_doBlit == 1) {
			frontend_video_present((uint32_t*)vga_framebuffer, (int)vga_w, (int)vga_h, 1024 * (int)sizeof(uint32_t));
			vga_doBlit = 0;
		}
		else {
			host_sleep(1);
...
static uint8_t vga_host_chain4_enabled() {
	return (vga_seqd[0x04] & 0x08) ? 1 : 0;
}

static uint8_t vga_host_odd_even_write_enabled() {
	return (!vga_host_chain4_enabled() && ((vga_seqd[0x04] & 0x04) == 0)) ? 1 : 0;
}

static uint8_t vga_host_odd_even_read_enabled() {
	return (!vga_host_chain4_enabled() && (vga_gfxd[0x05] & 0x10) && (vga_gfxd[0x06] & 0x02)) ? 1 : 0;
}

static void vga_loadlatches(uint32_t addr) {
	addr &= 0xFFFF;
...
void vga_writememory(void* dummy, uint32_t addr, uint8_t value) {
	uint8_t temp, plane, bitmask;
	if ((vga_misc & 0x02) == 0) return; //RAM writes are disabled
	addr -= 0xA0000;
	addr = (addr - vga_membase) & vga_memmask; //TODO: Is this right?

	if (vga_host_chain4_enabled()) { //chain-4
		plane = addr & 3;
		addr = (addr >> 2) & 0xFFFF;
		if (vga_enableplane & (1 << plane)) {
			vga_RAM[plane][addr] = value;
		}
		return;
	}

	if (vga_host_odd_even_write_enabled()) { //host odd/even mode
		uint8_t parity = addr & 1;

		addr = (addr >> 1) & 0xFFFF;
		if (vga_enableplane & (1 << parity)) {
			vga_RAM[parity][addr] = value;
		}
		if (vga_enableplane & (1 << (parity + 2))) {
			vga_RAM[parity + 2][addr] = value;
		}
		return;
	}
	...
	case 2:
		...
		break;
	case 3:
		bitmask = vga_dorotate(value) & vga_gfxd[0x08];
		for (plane = 0; plane < 4; plane++) {
			if (vga_enableplane & (1 << plane)) {
				temp = (vga_gfxd[0x00] & (1 << plane)) ? 0xFF : 0x00;
				temp = vga_dologic(temp, vga_latch[plane]);
				temp = (temp & bitmask) | (vga_latch[plane] & (~bitmask));
				vga_RAM[plane][addr] = temp;
			}
		}
		break;
...
uint8_t vga_readmemory(void* dummy, uint32_t addr) {
	...
	else if (vga_host_odd_even_read_enabled()) { //host odd/even mode
		plane = (addr & 1) | (vga_readmap & 0x02);
		addr = (addr >> 1) & 0xFFFF;
	}
	else {
		plane = vga_readmap & 3;
	...
	if (vga_rmode == 0) {
		...
		for (plane = 0; plane < 4; plane++) {
			if (vga_gfxd[0x07] & (1 << plane)) { //color don't care bit check
				uint8_t compare = (vga_gfxd[0x02] & (1 << plane)) ? 0xFF : 0x00;
				ret &= (uint8_t)~(vga_latch[plane] ^ compare);
			}
		}
		return ret;
	}
