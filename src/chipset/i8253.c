...
static void i8253_load_counter(I8253_t* i8253, uint8_t channel) {
	if (i8253->reload[channel] == 0) {
		i8253->reload[channel] = 65536;
	}
	i8253->counter[channel] = i8253->reload[channel];
...
void i8253_write(I8253_t* i8253, uint16_t portnum, uint8_t value) {
	...
	case 2:
		...
		case 1: //LSB only
			...
			i8253_load_counter(i8253, portnum);
			loaded = 1;
			break;
		case 2: //MSB only
