...
#define PCI_INTA           1
#define cpu_pci_speed      33333333.0

#define io_removehandler(base, count, readb, readw, readl, writeb, writew, writel, priv) \
    ports_cbUnregister((base), (count))
#define io_sethandler(base, count, readb, readw, readl, writeb, writew, writel, priv) \
    ports_cbRegisterEx((base), (count), (readb), (readw), (readl), (writeb), (writew), (writel), (priv))
#define mem_mapping_disable(mapping) memory_mapping_disable((mapping))
#define mem_mapping_enable(mapping) memory_mapping_enable((mapping))
#define mem_mapping_set_addr(mapping, start, len) memory_mapping_set_addr((mapping), (start), (len))

#define pci_set_irq(slot, intpin, stateptr) pci_raise_irq(s->pci, (slot), (intpin))
#define pci_clear_irq(slot, intpin, stateptr) pci_lower_irq(s->pci, (slot), (intpin))

typedef struct RTL8139State netcard_t;
...
static int rtl8139_do_receive(void *priv, uint8_t *buf, int size_);
static int rtl8139_network_rx_loopback(netcard_t *card, uint8_t *buf, int len);
static void rtl8139_network_tx(netcard_t *card, uint8_t *buf, int len);
static void rtl8139_host_rx(void *opaque, const uint8_t *data, int len);
static void rtl8139_eeprom_write(RTL8139EEPROM_t *eeprom, bool eecs, bool eesk, bool eedi);
static bool rtl8139_eeprom_read(RTL8139EEPROM_t *eeprom);
static void rtl8139_sync_tctr(struct RTL8139State *s);
static uint16_t rtl8139_read_u16_unaligned(const uint8_t *src);
static void rtl8139_write_u16_unaligned(uint8_t *dst, uint16_t value);
static void rtl8139_disable_bar_mappings(struct RTL8139State *s);
static void rtl8139_update_bar_mappings(struct RTL8139State *s);
static void rtl8139_log(const char *fmt, ...);

#define network_rx_put(card, buf, len) rtl8139_network_rx_loopback((card), (buf), (len))
#define network_tx(card, buf, len) rtl8139_network_tx((card), (buf), (len))

#define dma_bm_read(addr, data, total_size, transfer_size) \
    dma_bm_read(s->dma, (addr), (data), (total_size), (transfer_size))
#define dma_bm_write(addr, data, total_size, transfer_size) \
    dma_bm_write(s->dma, (addr), (data), (total_size), (transfer_size))
...
struct RTL8139State {
    ...
    uint32_t mem_base;
    uint16_t io_base;
    uint8_t  io_handler_installed;

    uint32_t timer_id;
    uint64_t tctr_last_sync;
    uint32_t io_trace_count;

    MEMORY_MAPPING_t bar_mem;
    RTL8139EEPROM_t  eeprom;
};

static uint16_t
rtl8139_get_io_base(const RTL8139State *s)
{
    return (uint16_t)(s->pci_conf[0x11] << 8);
}

static uint8_t
rtl8139_io_bar_is_active(const RTL8139State *s)
{
    uint16_t base = rtl8139_get_io_base(s);

    /* Ignore reset and BAR-size-probe values until firmware assigns a real base. */
    return (uint8_t)((base != 0x0000) && (base != 0xff00));
}

static uint32_t
rtl8139_get_mem_base(const RTL8139State *s)
{
#ifdef USE_256_BYTE_BAR
    return (s->pci_conf[0x15] << 8) | (s->pci_conf[0x16] << 16) |
           (s->pci_conf[0x17] << 24);
#else
    return ((s->pci_conf[0x15] & 0xf0) << 8) | (s->pci_conf[0x16] << 16) |
           (s->pci_conf[0x17] << 24);
#endif
}

static uint32_t
rtl8139_get_mem_bar_len(void)
{
#ifdef USE_256_BYTE_BAR
    return 256;
#else
...
static uint8_t
rtl8139_mem_bar_is_active(const RTL8139State *s)
{
    uint32_t base = rtl8139_get_mem_base(s);

#ifdef USE_256_BYTE_BAR
    return (uint8_t)((base != 0x00000000U) && (base != 0xffffff00U));
#else
    /* Avoid decoding over the top-of-ROM alias during BAR sizing writes. */
    return (uint8_t)((base != 0x00000000U) && (base != 0xfffff000U));
#endif
}
...
rtl8139_log(const char *fmt, ...)
{
    ...
    debug_log(DEBUG_DETAIL, "[RTL8139] %s", buffer);
}

static void
rtl8139_trace_access(RTL8139State *s, const char *space, const char *op, uint32_t addr, uint32_t value, uint32_t width)
{
    if (s == NULL) {
        return;
    }

    if (s->io_trace_count >= 512) {
        return;
    }

    s->io_trace_count++;
    rtl8139_log("%s %s addr=%02X width=%u value=%08X\n", space, op, addr & 0xFF, width, value);
}
...
#define CP_RX_VLAN_TAG_MASK ((1 << 16) - 1)
        ...
        if (s->CpCmd & CPlusRxVLAN && be16_to_cpu(rtl8139_read_u16_unaligned(&buf[ETH_ALEN * 2])) == 0x8100) {
            ...
            rxdw1 |= CP_RX_TAVA | rtl8139_read_u16_unaligned(&dot1q_buf[ETHER_TYPE_LEN]);

            rtl8139_log("C+ Rx mode : extracted vlan tag with tci: "
                        "%u\n",
                        be16_to_cpu(rtl8139_read_u16_unaligned(&dot1q_buf[ETHER_TYPE_LEN])));
        } else {
            /* reset VLAN tag flag */
            rxdw1 &= ~CP_RX_TAVA;
        }
...
rtl8139_reset_phy(RTL8139State *s)
{
    ...
    /* preserve link state */
    s->BasicModeStatus |= (s->link_state & NET_LINK_DOWN) ? 0 : 0x04;
...
//    s->TxConfig |= HW_REVID(1, 0, 0, 0, 0, 0, 0); // RTL-8139  HasHltClk
    ...
    s->TimerInt  = 0;
    s->TCTR_base = 0;
    s->tctr_last_sync = timing_getCur();
#if 0
...
rtl8139_Cfg9346_read(RTL8139State *s)
{
    ...
    if (opmode == 0x80) {
        if (rtl8139_eeprom_read(&s->eeprom))
            ret |= 0x01;
        else
            ret &= ~0x01;
    }
...
static int
rtl8139_network_rx_loopback(netcard_t *card, uint8_t *buf, int len)
{
    return rtl8139_do_receive(card, buf, len);
}

static void
rtl8139_network_tx(netcard_t *card, uint8_t *buf, int len)
{
    (void)card;
    hostnet_tx(buf, len);
}
...
static void
rtl8139_host_rx(void *opaque, const uint8_t *data, int len)
{
    rtl8139_do_receive(opaque, (uint8_t *)data, len);
}

void
rtl8139_network_rx_put(netcard_t *card, uint8_t *bufp, int len)
{
    (void) rtl8139_network_rx_loopback(card, bufp, len);
}
...
{
    void (*network_func)(netcard_t *, uint8_t *, int) = (TxLoopBack == (s->TxConfig & TxLoopBack)) ? rtl8139_network_rx_put : rtl8139_network_tx;
    if (!size) {
...
ones_complement_sum(uint8_t *data, size_t len)
{
    ...
    for (; len > 1; data += 2, len -= 2) {
        result += rtl8139_read_u16_unaligned(data);
    }

    /* add the remainder byte */
    if (len) {
        uint8_t odd[2] = { *data, 0 };
        result += rtl8139_read_u16_unaligned(odd);
    }
...
#define CP_TX_STATUS_EXC (1 << 20)
    ...
    if (txdw0 & CP_TX_LS) {
        ...
        if (txdw1 & CP_TX_TAGC) {
            ...
            dot1q_buffer = dot1q_buffer_space;
            rtl8139_write_u16_unaligned(dot1q_buffer, cpu_to_be16(0x8100));
            /* BE + le_to_cpu() + ~cpu_to_le()~ = BE */
            rtl8139_write_u16_unaligned(dot1q_buffer + ETHER_TYPE_LEN,
        ...
        if (txdw0 & (CP_TX_IPCS | CP_TX_UDPCS | CP_TX_TCPCS | CP_TX_LGSEN)) {
            ...
            int proto = be16_to_cpu(rtl8139_read_u16_unaligned(saved_buffer + 12));
            if (proto != ETH_P_IP) {
                goto skip_offload;
            }
            ...
            if ((txdw0 & CP_TX_LGSEN) && ip_protocol == IP_PROTO_TCP) {
                ...
                for (int tcp_send_offset = 0; tcp_send_offset < tcp_data_len; tcp_send_offset += large_send_mss) {
                    ...
                    if (tcp_send_offset) {
                        memmove((uint8_t *) p_tcp_hdr + tcp_hlen,
...
rtl8139_CSCR_read(RTL8139State *s)
{
    ...
    uint16_t ret = s->CSCR |
                   ((s->link_state & NET_LINK_DOWN) ? 0 : CSCR_Cable);
...
rtl8139_io_writeb(uint32_t addr, uint8_t val, void *priv)
{
    ...
    if ((addr >= (MAC0 + 6)) && (addr < (MAC0 + 8))) {
        return;
    }
    if ((addr >= MAR0) && (addr < (MAR0 + 8))) {
...
rtl8139_io_writel(uint32_t addr, uint32_t val, void *priv)
{
    ...
    addr &= 0xFF;
    if ((addr >= TxStatus0) && (addr < (TxStatus0 + (4 * 4)))) {
        rtl8139_TxStatus_write(s, addr - TxStatus0, val);
        return;
    }
    if ((addr >= TxAddr0) && (addr < (TxAddr0 + (4 * 4)))) {
...
rtl8139_io_readb(uint32_t addr, void *priv)
{
    ...
    addr &= 0xFF;
    if (addr < (MAC0 + 6)) {
        return s->phys[addr - MAC0];
    }
    if ((addr >= (MAC0 + 6)) && (addr < (MAC0 + 8))) {
        return 0;
    }
    if ((addr >= MAR0) && (addr < (MAR0 + 8))) {
        return s->mult[addr - MAR0];
    }
    if ((addr >= TxStatus0) && (addr < (TxStatus0 + (4 * 4)))) {
        return (uint8_t)rtl8139_TxStatus_TxAddr_read(s, s->TxStatus, TxStatus0, (uint8_t)addr, 1);
    }

    switch (addr) {
        case ChipCmd:
            ret = (uint8_t)rtl8139_ChipCmd_read(s);
            break;
        case Cfg9346:
            ret = (uint8_t)rtl8139_Cfg9346_read(s);
            break;
        case Config0:
            ret = (uint8_t)rtl8139_Config0_read(s);
            break;
        case Config1:
            ret = (uint8_t)rtl8139_Config1_read(s);
            break;
        case Config3:
            ret = (uint8_t)rtl8139_Config3_read(s);
            break;
        case Config4:
            ret = (uint8_t)rtl8139_Config4_read(s);
            break;
        case Config5:
            ret = (uint8_t)rtl8139_Config5_read(s);
            break;
        case MediaStatus:
            ret = (uint8_t)(0xd0 | (~s->BasicModeStatus & 0x04));
            break;
        case HltClk:
            ret = s->clock_enabled;
            break;
        ...
        case TxThresh:
            ...
            break;
        case 0x43:
            ret = (uint8_t)(s->TxConfig >> 24);
            break;
        case CSCR:
            ret = (uint8_t)(rtl8139_CSCR_read(s) & 0xff);
            break;
        case CSCR + 1:
            ret = (uint8_t)(rtl8139_CSCR_read(s) >> 8);
            break;
        default:
            ret = 0;
            break;
...
rtl8139_io_readw(uint32_t addr, void *priv)
{
    ...
    addr &= 0xFF;
    if ((addr >= TxAddr0) && (addr < (TxAddr0 + (4 * 4)))) {
        return (uint16_t)rtl8139_TxStatus_TxAddr_read(s, s->TxAddr, TxAddr0, (uint8_t)addr, 2);
    }

    switch (addr) {
        case IntrMask:
            ret = (uint16_t)rtl8139_IntrMask_read(s);
            break;
        case IntrStatus:
            ret = (uint16_t)rtl8139_IntrStatus_read(s);
            break;
        case MultiIntr:
            ret = (uint16_t)rtl8139_MultiIntr_read(s);
            break;
        case RxBufPtr:
            ret = (uint16_t)rtl8139_RxBufPtr_read(s);
            break;
        case RxBufAddr:
            ret = (uint16_t)rtl8139_RxBufAddr_read(s);
            break;
        case BasicModeCtrl:
            ret = (uint16_t)rtl8139_BasicModeCtrl_read(s);
            break;
        case BasicModeStatus:
            ret = (uint16_t)rtl8139_BasicModeStatus_read(s);
            break;
        case NWayAdvert:
            ret = s->NWayAdvert;
            break;
        ...
        case NWayExpansion:
            ...
            break;
        case CpCmd:
            ret = (uint16_t)rtl8139_CpCmd_read(s);
            break;
        case IntrMitigate:
            ret = (uint16_t)rtl8139_IntrMitigate_read(s);
            break;
        case TxSummary:
        ...
        default:
            ...
            ret |= (uint16_t)rtl8139_io_readb(addr + 1, priv) << 8;
            break;
    }
...
rtl8139_io_readl(uint32_t addr, void *priv)
{
    ...
    addr &= 0xFF;
    if ((addr >= TxStatus0) && (addr < (TxStatus0 + (4 * 4)))) {
        return rtl8139_TxStatus_TxAddr_read(s, s->TxStatus, TxStatus0, (uint8_t)addr, 4);
    }
    if ((addr >= TxAddr0) && (addr < (TxAddr0 + (4 * 4)))) {
        return rtl8139_TxAddr_read(s, addr - TxAddr0);
    }
    ...
    switch (addr) {
        ...
        default:
            ret = rtl8139_io_readb(addr, priv);
            ret |= (uint32_t)rtl8139_io_readb(addr + 1, priv) << 8;
            ret |= (uint32_t)rtl8139_io_readb(addr + 2, priv) << 16;
            ret |= (uint32_t)rtl8139_io_readb(addr + 3, priv) << 24;
            break;
    }
...
static uint32_t
rtl8139_io_readl_ioport(void *priv, uint16_t addr)
{
    RTL8139State *s = (RTL8139State *) priv;
    uint32_t ret = rtl8139_io_readl(addr, priv);

    rtl8139_trace_access(s, "IO", "read", addr, ret, 4);
    return ret;
}

static uint16_t
rtl8139_io_readw_ioport(void *priv, uint16_t addr)
{
    RTL8139State *s = (RTL8139State *) priv;
    uint16_t ret = rtl8139_io_readw(addr, priv);

    rtl8139_trace_access(s, "IO", "read", addr, ret, 2);
    return ret;
}

static uint8_t
rtl8139_io_readb_ioport(void *priv, uint16_t addr)
{
    RTL8139State *s = (RTL8139State *) priv;
    uint8_t ret = rtl8139_io_readb(addr, priv);

    rtl8139_trace_access(s, "IO", "read", addr, ret, 1);
    return ret;
}

static void
rtl8139_io_writel_ioport(void *priv, uint16_t addr, uint32_t val)
{
    rtl8139_trace_access((RTL8139State *) priv, "IO", "write", addr, val, 4);
    rtl8139_io_writel(addr, val, priv);
}

static void
rtl8139_io_writew_ioport(void *priv, uint16_t addr, uint16_t val)
{
    rtl8139_trace_access((RTL8139State *) priv, "IO", "write", addr, val, 2);
    rtl8139_io_writew(addr, val, priv);
}

static void
rtl8139_io_writeb_ioport(void *priv, uint16_t addr, uint8_t val)
{
    rtl8139_trace_access((RTL8139State *) priv, "IO", "write", addr, val, 1);
    rtl8139_io_writeb(addr, val, priv);
}

static uint32_t
rtl8139_io_readl_mem(void *priv, uint32_t addr)
{
    RTL8139State *s = (RTL8139State *) priv;
    ...
    if ((addr >= s->mem_base) && (addr < (s->mem_base + 0xff))) {
        ...
        rtl8139_trace_access(s, "MMIO", "read", addr - s->mem_base, ret, 4);
    }
...
static uint16_t
rtl8139_io_readw_mem(void *priv, uint32_t addr)
{
    RTL8139State *s = (RTL8139State *) priv;
    uint16_t ret = 0xffff;
    ...
    if ((addr >= s->mem_base) && (addr < (s->mem_base + 0xff))) {
        ...
        rtl8139_trace_access(s, "MMIO", "read", addr - s->mem_base, ret, 2);
    }
...
static uint8_t
rtl8139_io_readb_mem(void *priv, uint32_t addr)
{
    RTL8139State *s = (RTL8139State *) priv;
    uint8_t ret = 0xff;
    ...
    if ((addr >= s->mem_base) && (addr < (s->mem_base + 0xff))) {
        ret = rtl8139_io_readb(addr, priv);
        rtl8139_trace_access(s, "MMIO", "read", addr - s->mem_base, ret, 1);
    }
...
static void
rtl8139_io_writel_mem(void *priv, uint32_t addr, uint32_t val)
{
    RTL8139State *s = (RTL8139State *) priv;

    if ((addr >= s->mem_base) && (addr < (s->mem_base + 0xff))) {
        rtl8139_trace_access(s, "MMIO", "write", addr - s->mem_base, val, 4);
        rtl8139_io_writel(addr, val, priv);
    }
}

static void
rtl8139_io_writew_mem(void *priv, uint32_t addr, uint16_t val)
{
    RTL8139State *s = (RTL8139State *) priv;

    if ((addr >= s->mem_base) && (addr < (s->mem_base + 0xff))) {
        rtl8139_trace_access(s, "MMIO", "write", addr - s->mem_base, val, 2);
        rtl8139_io_writew(addr, val, priv);
    }
}

static void
rtl8139_io_writeb_mem(void *priv, uint32_t addr, uint8_t val)
{
    RTL8139State *s = (RTL8139State *) priv;

    if ((addr >= s->mem_base) && (addr < (s->mem_base + 0xff))) {
        rtl8139_trace_access(s, "MMIO", "write", addr - s->mem_base, val, 1);
        rtl8139_io_writeb(addr, val, priv);
    }
}

static void
rtl8139_eeprom_write(RTL8139EEPROM_t *eeprom, bool eecs, bool eesk, bool eedi)
{
    int cs  = eecs ? 1 : 0;
    int clk = eesk ? 1 : 0;
    int di  = eedi ? 1 : 0;

    if (cs && !eeprom->cs_prev) {
        eeprom->state   = 0;
        eeprom->bits    = 0;
        eeprom->command = 0;
        eeprom->do_bit  = 0;
    }
    eeprom->cs_prev = (uint8_t)cs;

    if (!cs) {
        eeprom->do_bit = 0;
        eeprom->clk_prev = (uint8_t)clk;
        return;
    }

    if (clk && !eeprom->clk_prev) {
        switch (eeprom->state) {
            case 0:
                if (di) {
                    eeprom->state   = 1;
            ...
            case 1:
                eeprom->command = (uint8_t)((eeprom->command << 1) | di);
                eeprom->bits++;
                if (eeprom->bits >= 8) {
                    int cmd  = (eeprom->command >> 6) & 0x3;
                    int addr = eeprom->command & 0x3F;

                    if (cmd == 2) {
                        eeprom->data  = eeprom->words[addr];
                        eeprom->bits  = 16;
                        eeprom->state = 2;
                        eeprom->do_bit = 0;
                    } else {
            ...
            case 2:
                if (eeprom->bits > 0) {
                    eeprom->do_bit = (uint8_t)((eeprom->data >> 15) & 1);
                    eeprom->data <<= 1;
                    eeprom->bits--;
                } else {
                    eeprom->do_bit = 0;
...
static bool
rtl8139_eeprom_read(RTL8139EEPROM_t *eeprom)
{
    return eeprom->do_bit != 0;
}

static uint16_t
rtl8139_read_u16_unaligned(const uint8_t *src)
{
    uint16_t value;
...
static void
rtl8139_write_u16_unaligned(uint8_t *dst, uint16_t value)
{
    memcpy(dst, &value, sizeof(value));
}

static void
rtl8139_sync_tctr(RTL8139State *s)
{
    uint64_t now;
    uint64_t delta;
    uint64_t increment;
    ...
    now = timing_getCur();
    if (s->tctr_last_sync == 0) {
        s->tctr_last_sync = now;
        return;
    }
    ...
    delta = now - s->tctr_last_sync;
    if (delta == 0) {
        return;
    }

    increment = (delta * 33333333ULL) / timing_getFreq();
    if (increment == 0) {
        return;
    }

    old_tctr = s->TCTR;
    s->TCTR += (uint32_t)increment;
    s->tctr_last_sync = now;

    if (s->TimerInt != 0) {
        if ((old_tctr < s->TimerInt && s->TCTR >= s->TimerInt) ||
            (s->TCTR < old_tctr && (s->TimerInt > old_tctr || s->TimerInt <= s->TCTR))) {
...
static void
rtl8139_disable_bar_mappings(RTL8139State *s)
{
    rtl8139_log("Disabling BAR decodes (io=%04X mem=%08X cmd=%02X)\n",
                rtl8139_get_io_base(s), s->mem_base, s->pci_conf[0x04]);

    if (s->bar_mem.enabled) {
        mem_mapping_disable(&s->bar_mem);
    }
    if (s->io_handler_installed) {
        io_removehandler(s->io_base, 256,
                         ...
                         s);
        s->io_handler_installed = 0;
        s->io_base = 0;
    }
}

static void
rtl8139_update_bar_mappings(RTL8139State *s)
{
    uint16_t io_base;

    rtl8139_disable_bar_mappings(s);

    io_base = rtl8139_get_io_base(s);
    if ((s->pci_conf[0x04] & PCI_COMMAND_IO) && rtl8139_io_bar_is_active(s)) {
        rtl8139_log("Enabling I/O decode at %04X\n", io_base);
        io_sethandler(io_base, 256,
                      rtl8139_io_readb_ioport, rtl8139_io_readw_ioport, rtl8139_io_readl_ioport,
                      rtl8139_io_writeb_ioport, rtl8139_io_writew_ioport, rtl8139_io_writel_ioport,
                      s);
        s->io_base = io_base;
        s->io_handler_installed = 1;
    } else if (s->pci_conf[0x04] & PCI_COMMAND_IO) {
        rtl8139_log("Leaving I/O decode disabled for BAR value %04X\n", io_base);
    }

    if ((s->pci_conf[0x04] & PCI_COMMAND_MEM) && rtl8139_mem_bar_is_active(s)) {
        rtl8139_log("Enabling MMIO decode at %08X len=%u\n", s->mem_base,
                    (unsigned) rtl8139_get_mem_bar_len());
        mem_mapping_set_addr(&s->bar_mem, s->mem_base, rtl8139_get_mem_bar_len());
        mem_mapping_enable(&s->bar_mem);
    } else if (s->pci_conf[0x04] & PCI_COMMAND_MEM) {
        rtl8139_log("Leaving MMIO decode disabled for BAR value %08X\n", s->mem_base);
    }
}
...
static uint8_t
rtl8139_pci_read(void *priv, uint8_t func, uint8_t addr)
{
    ...
    if ((addr <= 0x17) || (addr == 0x3c) || (addr == 0x3d)) {
        rtl8139_log("PCI read addr=%02X -> %02X\n", addr, ret);
    }
...
static void
rtl8139_pci_write(void *priv, uint8_t func, uint8_t addr, uint8_t val)
{
    RTL8139State *s = (RTL8139State *) priv;
    (void)func;

    if ((addr <= 0x17) || (addr == 0x3c) || (addr == 0x3d)) {
        rtl8139_log("PCI write addr=%02X val=%02X\n", addr, val);
    }

    switch (addr) {
        case 0x04:
            s->pci_conf[addr & 0xFF] = val;
            rtl8139_update_bar_mappings(s);
            break;
        case 0x05:
        ...
        case 0x11:
            s->pci_conf[addr & 0xFF] = val;
            rtl8139_log("New I/O base: %04X\n", rtl8139_get_io_base(s));
            if (s->pci_conf[0x04] & PCI_COMMAND_IO) {
                rtl8139_update_bar_mappings(s);
            }
            break;
        ...
        case 0x17:
            s->pci_conf[addr & 0xFF] = val;
            s->mem_base = rtl8139_get_mem_base(s);
...
int
rtl8139_init(RTL8139State **out_dev, PCI_t *pci, DMA_t *dma, uint8_t pci_slot, const uint8_t *mac, int pcap_if)
{
    RTL8139State *s;
    uint16_t     *eep_data;

    if ((out_dev == NULL) || (pci == NULL) || (dma == NULL) || (mac == NULL)) {
        return -1;
    }

    s = calloc(1, sizeof(RTL8139State));
    if (s == NULL) {
        return -1;
    }
    ...
    s->pci_slot = pci_slot;
    s->nic = s;
    s->link_state = NET_LINK_DOWN;

    debug_log(DEBUG_DETAIL,
              "[RTL8139] Initializing PCI RTL8139 at slot %02X with (MAC %02X:%02X:%02X:%02X:%02X:%02X)\r\n",
              (unsigned)pci_slot,
              (unsigned)mac[0], (unsigned)mac[1], (unsigned)mac[2],
              (unsigned)mac[3], (unsigned)mac[4], (unsigned)mac[5]);

    memory_mapping_add(&s->bar_mem, 0, 0,
                       ...
                       s, 0);
    pci_register_device(pci, pci_slot, 0, rtl8139_pci_read, rtl8139_pci_write, s);

    eep_data = s->eeprom.words;
    memset(eep_data, 0, sizeof(s->eeprom.words));

    eep_data[0] = 0x8129;
    eep_data[1] = 0x10EC;
    eep_data[2] = 0x8139;
    eep_data[7] = (uint16_t)(mac[0] | ((uint16_t)mac[1] << 8));
    eep_data[8] = (uint16_t)(mac[2] | ((uint16_t)mac[3] << 8));
    eep_data[9] = (uint16_t)(mac[4] | ((uint16_t)mac[5] << 8));

    memcpy(s->phys, mac, 6);
    s->timer_id = timing_addTimer(rtl8139_timer, s, 1000.0, TIMING_ENABLED);
    s->tctr_last_sync = timing_getCur();

    rtl8139_reset(s);

    if (pcap_if > -1) {
        if (hostnet_attach(pcap_if, rtl8139_host_rx, s, rtl8139_set_link_status, s) != 0) {
            free(s);
            return -1;
        }
    }
...
void
rtl8139_reset_device(RTL8139State *dev)
{
    if (dev == NULL) {
        return;
    }
