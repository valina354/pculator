...
static void ne2000_raise_irq(NE2000_t* ne2000)
{
    ...
    if ((ne2000->isa_irq >= 0) && (ne2000->isa_irq < 8)) {
        if (ne2000->pic_master != NULL) {
            i8259_doirq(ne2000->pic_master, (uint8_t)ne2000->isa_irq);
        }
        return;
    }

    if ((ne2000->isa_irq >= 8) && (ne2000->isa_irq < 16) && (ne2000->pic_slave != NULL)) {
        i8259_doirq(ne2000->pic_slave, (uint8_t)(ne2000->isa_irq - 8));
    }
}
//
...
uint16_t ne2000_chipmem_read_w(NE2000_t* ne2000, uint32_t address)
{
    // ROM'd MAC address
    if ((address >= 0) && (address <= 31)) {
        return host_read_le16_unaligned(ne2000->macaddr + address);
    }

    if ((address >= NE2K_MEMSTART) && (address < NE2K_MEMEND)) {
        return host_read_le16_unaligned(ne2000->mem + (address - NE2K_MEMSTART));
    }
    else {
        return (0xffff);
    }
...
{
    if ((address >= NE2K_MEMSTART) && (address < NE2K_MEMEND)) {
        host_write_le16_unaligned(ne2000->mem + (address - NE2K_MEMSTART), value);
    }
}
...
uint16_t ne2000_asic_read_w(void* priv, uint16_t offset)
{
...
void ne2000_asic_write_w(void* priv, uint16_t offset, uint16_t value)
{
    NE2000_t* ne2000 = (NE2000_t*)priv;
    (void)offset;
#ifdef DEBUG_NE2000
...
uint8_t ne2000_asic_read_b(void* priv, uint16_t offset)
{
    NE2000_t* ne2000 = (NE2000_t*)priv;
    if (offset & 1)
...
void ne2000_asic_write_b(void* priv, uint16_t offset, uint8_t value)
{
    NE2000_t* ne2000 = (NE2000_t*)priv;
    if (offset & 1)
...
uint8_t ne2000_reset_read(void* priv, uint16_t offset)
{
    NE2000_t* ne2000 = (NE2000_t*)priv;
    (void)offset;
...
void ne2000_reset_write(void* priv, uint16_t offset, uint8_t value)
{
    (void)priv;
    (void)offset;
    (void)value;
...
//
uint8_t ne2000_read(void* priv, uint16_t address)
{
    NE2000_t* ne2000 = (NE2000_t*)priv;
    int ret;
...
// by this ne2000 instance
//
void ne2000_write(void* priv, uint16_t address, uint8_t value)
{
    NE2000_t* ne2000 = (NE2000_t*)priv;
#ifdef DEBUG_NE2000
    ...
    if (address == 0x00) {
        ...
        if ((value & 0x04) && ne2000->TCR.loop_cntl) {
            ...
            // Send the packet to the system driver
                hostnet_tx(&ne2000->mem[ne2000->tx_page_start * 256 - NE2K_MEMSTART], (int)ne2000->tx_bytes);
        ...
        case 0x00:
            ...
            case 0xf:  // IMR
              ...
              // Check for reserved bit
                ...
                if (value != 0) {
#ifdef DEBUG_NE2000
                    debug_log(DEBUG_DETAIL, "[NE2000] irq retrigger\n");
#endif
                    ne2000_raise_irq(ne2000);
                }
                break;
...
void ne2000_init(NE2000_t* ne2000, I8259_t* pic_master, I8259_t* pic_slave, uint32_t baseport, uint8_t irq, uint8_t* macaddr) {
    debug_log(DEBUG_DETAIL, "[NE2000] Initializing NE2000 Ethernet adapter at 0x%03X, IRQ %u (MAC %02X:%02X:%02X:%02X:%02X:%02X)\r\n",
        baseport, irq,
        (unsigned)macaddr[0], (unsigned)macaddr[1], (unsigned)macaddr[2],
        (unsigned)macaddr[3], (unsigned)macaddr[4], (unsigned)macaddr[5]);
    ne2000->pic_master = pic_master;
    ne2000->pic_slave = pic_slave;
    ne2000->base_address = baseport;
